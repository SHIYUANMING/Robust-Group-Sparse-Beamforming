...
LC=30; %# loops for channel realizarions
...
M=2; %Number of Multicast Groups
...
if feasible_CBF==1
TotalPower_CBF(lq)=(1/amcoeff)*norm(V_CBF,'fro')^2+sum(Pc);
TransmitPower_CBF(lq)=(1/amcoeff)*norm(V_CBF,'fro')^2;
else
      ...
      %  [BS,BS_index]=sort(Value_GSBF);
        ...
        for A=0:L-2
            ...
            if feasible==1
                ...
                TotalPower_GSBFAlternating(lq)=(1/amcoeff)*norm(Wsolution,'fro')^2+sum(Pc(A_set));  %recode current values
                TransmitPower_GSBFAlternating(lq)=(1/amcoeff)*norm(Wsolution,'fro')^2;
...
%         [BS,BS_index]=sort(ChannelGain.*Value_GSBF);
        ...
        for A=0:L-2
            ...
            if feasible==1
                ...
                TotalPower_Baseline(lq)=(1/amcoeff)*norm(Wsolution,'fro')^2+sum(Pc(A_set));  %recode current values
                TransmitPower_Baseline(lq)=(1/amcoeff)*norm(Wsolution,'fro')^2;
