function [Vsolution,feasible] = baseline_cvx(params)
...
variable T(length(Active_index),length(Active_index));  %slack variables for each beamforming matrix for the RAUs
variable lambda(length(K_set), K_set(1));  % MxK variables for the S-Lemma in the QoS constraints
minimize sum(vec(T))  % group sparsity inducing minimization
subject to

%% RAUs Transmit Power Constraints

%% Objective Function
% for l_1=1:length(Active_index)  
%     for l_2=1:length(Active_index)
%         for m=1:length(K_set)
%             max(vec(abs(Q(sum(N_set(1:Active_index(l_1)-1))+1:sum(N_set(1:Active_index(l_1))),sum(N_set(1:Active_index(l_2)-1))+1:sum(N_set(1:Active_index(l_2))),m))))<=T(l_1,l_2,m);
%             T(l_1,l_2,m)>=0;
%         end
%     end
% end
...
for l_1=1:length(Active_index)  
    for l_2=1:length(Active_index)
        %for m=1:length(K_set)
            max(vec(abs(Q(sum(N_set(1:Active_index(l_1)-1))+1:sum(N_set(1:Active_index(l_1))),sum(N_set(1:Active_index(l_2)-1))+1:sum(N_set(1:Active_index(l_2))),:))))<=T(l_1,l_2);
            T(l_1,l_2)>=0;
        %end
    end
end
